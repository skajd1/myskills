...
저장소별 `.commit-message.yml`을 우선 적용해 구조화된 한국어 커밋 메시지와 릴리즈노트 메타데이터를 작성합니다.
...
- Git이 추적할 개인 스킬을 추가, 삭제, 이름 변경하면 이 파일의 스킬 인덱스도 함께 갱신합니다.
- `.gitignore`에 등록된 로컬 전용 스킬은 이 파일과 `README.md`의 스킬 인덱스에 포함하지 않습니다. 기존 스킬을 `.gitignore`에 추가할 때는 두 인덱스에서도 함께 제거합니다.
- 이 저장소에서 스킬 변경이 발생하고 GitHub 반영이 필요하면 `skill-manager` 스킬을 사용해 검증, 선택적 staging, commit, push 절차를 진행합니다.
- 이 저장소에서 파일 변경이 발생하면 항상 사용자에게 커밋 여부, 제안 커밋 메시지, 커밋 후 push 여부를 함께 물어보고 명시적 승인을 받은 뒤 commit/push 합니다.
